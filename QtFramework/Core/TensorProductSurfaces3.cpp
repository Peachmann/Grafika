...
TensorProductSurface3::TensorProductSurface3(GLdouble u_min, GLdouble u_max, GLdouble v_min, GLdouble v_max,
                                             GLuint row_count, GLuint column_count, GLboolean u_closed, GLboolean v_closed):
        _u_min(u_min),_u_max(u_max),_v_min(v_min),_v_max(v_max),
        _u_closed(u_closed), _v_closed(v_closed), _data(Matrix<DCoordinate3>(row_count,column_count))
{
}
...
TensorProductSurface3::TensorProductSurface3(const TensorProductSurface3 &surface):
        ...
        _u_closed(surface._u_closed), _v_closed(surface._v_closed), _vbo_data(surface._vbo_data),_data(surface._data)
{
    //asszem
...
// homework set the definition domain of the surface
...
// homework get the definition domain of the surface
...
{
    if(_data.GetRowCount() < row || _data.GetColumnCount() < column)
        return GL_FALSE;
...
GLboolean TensorProductSurface3::SetData(GLuint row, GLuint column, const DCoordinate3 &point)
{
    ...
    _data(row,column)[0] = point[0];
    _data(row,column)[1] = point[1];
    _data(row,column)[2] = point[2];
...
GLboolean TensorProductSurface3::GetData(GLuint row, GLuint column, DCoordinate3 &point) const
{
    if(_data.GetRowCount() < row || _data.GetColumnCount() < column)
        return GL_FALSE;
/*
    point[0] = _data(row,column)[0];
    point[1] = _data(row,column)[1];
    point[2] = _data(row,column)[2];
*/
    ...
    if (!_vbo_data)
        ...
        glEnableClientState(GL_VERTEX_ARRAY);
            glBindBuffer(GL_ARRAY_BUFFER, _vbo_data);
                ...
                // ! nem mindegy a sorrend , mert ilyen sorrendbe tettuk be a buffer-be
                // soronkénti bejárás
                                                      ...
                                                      GLenum usage_flag ) const
{
    ...
    for(GLuint i = 0; i < iso_line_count; i++)
    {
        ...
        for(GLuint j = 0;j < div_point_count; j++)
        {
            ...
            //ha a masodikat irjuk pd(r,0) helyett pd(r,r) ->foatlon vannak a V szerinti derivaltak
                                                      ...
                                                      GLenum usage_flag ) const
{
    ...
    for(GLuint i = 0; i < iso_line_count; i++)  //NEM DIV_POINT_COUNTIG | EZ NEM BIZTOS
    {
...
// homework: initializes all information above to null-vector
