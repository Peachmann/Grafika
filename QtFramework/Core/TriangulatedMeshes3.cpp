...
std::ostream& cagd::operator <<(std::ostream& lhs, const TriangulatedMesh3& rhs)
{
    ...
    for(GLuint i = 0; i < rhs._normal.size(); i++)
    {
        lhs << rhs._normal[i];
    }

    for(GLuint i = 0; i < rhs._tex.size(); i++)
    {
        lhs << rhs._tex[i];
    }
...
std::istream& cagd::operator >>(std::istream& lhs, TriangulatedMesh3& rhs)
{
   ...
   GLuint _vertexSize,_faceSize;

   DCoordinate3 d;
   TriangularFace f;
   TCoordinate4 t;
   lhs >> _vertexSize >> _faceSize;

   rhs._vertex.resize(_vertexSize);
   rhs._normal.resize(_vertexSize);
   rhs._tex.resize(_vertexSize);
   rhs._face.resize(_faceSize);

   for(GLuint i = 0; i < _vertexSize; i++)
   {
        lhs >> rhs._vertex[i];
   }
